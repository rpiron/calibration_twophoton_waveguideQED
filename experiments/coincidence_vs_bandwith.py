...
def run_coincidence_vs_bandwith(param_photons, param_cavity_physical, param_time_evol, ir_tab, uv_tab, 
                                index_omega_q = 0, index_experiment = 0, n= 1, store_results:bool=True, progress:bool=True):
    """
    To be completed
    """
    ...
    if store_results:
        ...
        df.to_csv(project_root / 'results' / 'csv_files' / 'coincidence_vs_bandwidth' / f'{index_experiment}' / f'coincidence_vs_bandwith_omega{index_omega_q}_xp{index_experiment}_n{n}.csv', index=False)
